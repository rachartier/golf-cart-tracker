...
class TimeSeriesDB:
    ...
    def get_car(self, car_id: str, count: int = 10) -> list[Car]:
        """
        Retrieve a list of Car objects for a given car_id, limited to a specified count.

        Args:
            car_id (str): The ID of the car to retrieve.
            count (int, optional): The maximum number of Car objects to retrieve. Defaults to 10.
        ...
        car_list = [Car(**result.fields) for result in results]

        return car_list

    def get_car_after(self, count_by_car: int, date_after: datetime) -> dict[str, list[Car]]:
        """
        Retrieve a dictionary of Car objects grouped by car_id, for entries after a specified date.

        Args:
            count_by_car (int): The maximum number of Car objects to retrieve per car_id.
            date_after (datetime): The date after which to retrieve Car objects.

        Returns:
            dict[str, list[Car]]: A dictionary where keys are car_ids and values are lists of Car objects.
        """
        ...
        grouped_results: dict[str, list[Car]] = {}
        ...
        for result in wanted_results:
            ...
            if len(grouped_results[result.tags["id"]]) < count_by_car:
                grouped_results[result.tags["id"]].append(Car(**result.fields))

        return grouped_results

    def get_cars_today(self, count_by_car: int):
        """
        Retrieve a dictionary of Car objects grouped by car_id, for entries from today.
    ...
    def insert(self, car_id: str, car: Car) -> None:
        """
